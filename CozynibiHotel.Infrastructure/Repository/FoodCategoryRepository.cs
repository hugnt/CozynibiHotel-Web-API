...
namespace CozynibiHotel.Infrastructure.Repository
{
    public class FoodCategoryRepository : GenericRepository<FoodCategory>, IFoodCategoryRepository
    {
        ...
        public FoodCategoryRepository(AppDbContext dbContext, IMapper mapper) : base(dbContext)
        {
            _dbContext = dbContext;
            _mapper = mapper;
        }
        public ICollection<FoodCategoryDto> Search(string field, string keyWords)
        {
            var all = _mapper.Map<List<FoodCategoryDto>>(GetAll());
        ...
        public bool SetDelete(int id, bool isDelete)
        {
            try
            {
                var selectedRecord = _dbContext.FoodCategories.Find(id);
